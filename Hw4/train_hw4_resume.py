...
支持从checkpoint继续训练 (Resume from checkpoint)
"""
import subprocess
...
def main():
    parser = argparse.ArgumentParser(description='Train PromptIR for Image Restoration (with Resume support)')
    ...
    parser.add_argument('--resume', type=str, default=None,
                        help='Resume from checkpoint (path to .ckpt file)')
    parser.add_argument('--auto_resume', action='store_true',
                        help='Automatically resume from latest checkpoint in ckpt_dir')
    ...
    resume_ckpt = None
    if args.auto_resume:
        resume_ckpt = find_latest_checkpoint(args.ckpt_dir)
        if resume_ckpt:
            print(f"[Auto Resume] Found latest checkpoint: {resume_ckpt}")
        else:
            print(f"[Auto Resume] No checkpoint found in {args.ckpt_dir}, starting from scratch")
    elif args.resume:
        if os.path.exists(args.resume):
            resume_ckpt = args.resume
            print(f"[Resume] Using checkpoint: {resume_ckpt}")
        else:
    ...
    # Create model
    if resume_ckpt:
        print(f"\nLoading model from checkpoint: {resume_ckpt}")
