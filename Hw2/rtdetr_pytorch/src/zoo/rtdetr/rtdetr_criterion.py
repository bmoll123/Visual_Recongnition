...
class SetCriterion(nn.Module):
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        # ========= 修改這裡 =========
        # 將原本的 generalized_box_iou 替換為 complete_box_iou
        loss_ciou = 1 - torch.diag(
            complete_box_iou(
                box_cxcywh_to_xyxy(src_boxes), box_cxcywh_to_xyxy(target_boxes)
            )
        )

        # 💡 重要踩坑提醒：這裡的 Dictionary Key 請「保持原名」叫 'loss_giou'！
        # 因為 RT-DETR 的 YAML config 檔裡面會用 'loss_giou' 這個字串去匹配權重 (weight_dict)。
        # 如果你把這裡改成 losses['loss_ciou']，程式在 forward 時會因為找不到對應的權重而報錯。
